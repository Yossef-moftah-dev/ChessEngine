...
public:
    ...
    static constexpr Score TablesBaseMate = 20000;
    static constexpr Score MateThreashold = 30000;
...
public:
    ...
    static constexpr CastlingPermission All = WhiteQueen | WhiteKing | BlackQueen | BlackKing;
private:
    CastlingPerissions() = default;
};
...
public:
    ...
    static constexpr Color ReflextedMask = ~Mask;
};
...
template<class T>
using EachPieceColor = std::array<T, 8>;
...
public:
    ...
    {
        auto relfected = std::array<T, Count>{};
        for(auto i=0; i < count; i++)
        {
            relfected[i] = arr[Reflected[i]];
        }
        return relfected;
    }

    template<class T>
    static constexpr EachColor<std::array<T, Count >> MakeReflective(const std::array<T, Count>& arr)
    {
    ...
    static constexpr EachColor<std::array<Position, Count>> Reflective = 
    {
        All,
        Reflected,
    };
...
public:
    ...
    static constexpr std::array<Bitboard, 15> Antidialgonals =
    {
        0x80ULL,
        0x8040ULL,
        0x804020ULL,
    ...
    static constexpr std::array<Bitboard, Files.size() + Ranks.size() + Diagonals.size() 
    + Antidialgonals.size()> AllSlides = 
    {
        0x101010101010101ULL,
        0x202020202020202ULL,
        0x404040404040404ULL,
    ...
    static constexpr Bitboard WhiteQueenSideCastleMask = (Files[1] | Files[2] | Files[3]) & Ranks[7];
    static constexpr Bitboard WhiteKingSideCastleMask = (Files[5] | Files[6]) & Ranks[7];
    ...
    static constexpr Bitboard WhiteQueenSideCastleAttackMask = (Files[2] | Files[3] | Files[4]) & Ranks[7];
    static constexpr Bitboard WhiteKingSideCastleAttackMask = (Files[4] | Files[5] | Files[6]) & Ranks[7];
...
{
public:
    static constexpr EachPiece<Score> PieceValues = {0, 0, 0, 0, 100, 100, 325, 325, 335, 335, 500, 
        500, 975, 975, 0, 0};
};
...
{
private:
    static constexpr Bitboard GetAttackedByJumpingPiece(const Position position, const Bitboard jumpMask,
    const Position jumpMaskCenter)
    {
        ...
        jumps &= ~(position % 8 < 4 ? BitBoardConstants::Files[6] | BitBoardConstants::Files[7] 
            : BitBoardConstants::Files[0] | BitBoardConstants::Files[1]);
        jumps &= ~((position >> 3) < 4 ? BitBoardConstants::Ranks[6] | BitBoardConstants::Ranks[7]
            : BitBoardConstants::Ranks[0] | BitBoardConstants::Ranks[1]);
        return jumps;
    }
...
public:
    ...
    constexpr BitboardJumpsClass()
    {
        ...
        constexpr std::array<Bitboard, 2> pawnPositions{1, 9};
        ...
        for(Position i = 0; i < Positions::Count; i++)
        {
            ...
            for(int j=0; j<2; j++)
            {
                ...
                const auto pawnSupportSpan = pawnSupportSpans[j];
                const auto pawnSupportPosition = pawnSupportPositions[j];
...
public:
    ...
    constexpr BetweenBitboardsClass()
    {
        for(Position i=0; i<Positions::Count; i++)
        {
            ...
            const Antidialgonal toAntidiagonal = fromRank - fromFile + 7;
            ...
            for(Position j=0; j<64; j++)
            {
                ...
                {
                    slide = BitBoardConstants::Antidialgonals[fromAntidiagonal];
                }
...
{
public:
    using ReductionsTableTypes = EachSearchPly<EachMove<Ply>>;

    ReductionsTableTypes Reductions{};
    ...
    SearchDataClass()
    {
        for(Ply depth = 1; depth <  Constants::MaxSearchPly; depth++)
        {
            ...
            {
                const double reduction = std::log(depth) * std::log(movesEvaluated) * 0.4 + 0.1;
